...
model = YOLO("root_path\\ultralytics\\cfg\models\\11\\yolo11-obb-MP.yaml")#模型配置文件路径
train_params = {
    'data': "root_path\\ultralytics\\cfg\\datasets\\HRSC.yaml",  # 数据集配置文件路径
    'epochs': 150,  # 总训练轮次，默认值 100，范围 >= 1
    ...
    'workers': 0,  # 数据加载线程数，默认值 8，影响数据预处理速度
